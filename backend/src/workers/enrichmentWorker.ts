...
async function processEnrichmentJob(
  ...
  try {
    ...
    // ⚠️ CRITICAL: Save enrichment results to database
    if (job.data.bookmarkId && job.data.userId) {
      ...
      try {
        ...
        await bookmarkRepository.update(job.data.bookmarkId, job.data.userId, {
          title: result.title,
          domain: result.domain, // Domain is a top-level field, not in metadata
          summary: result.analysis.summary,
        ...
        if (result.tagging?.tags && result.tagging.tags.length > 0) {
          console.log(`[Worker] 💾 Saving ${result.tagging.tags.length} tags for bookmark ${job.data.bookmarkId}`);

          // Import prisma for tag operations
          const prisma = (await import('../db/prisma')).default;

          // Normalize all tag names
          const tagNames = result.tagging.tags.map(t => t.toLowerCase().trim());

          // 1. Find all existing tags in one query (instead of one query per tag)
          const existingTags = await prisma.tag.findMany({
            where: {
              userId: job.data.userId,
              normalizedName: { in: tagNames }
            }
          });

          // 2. Batch create missing tags
          const existingNames = new Set(existingTags.map(t => t.normalizedName));
          const missingNames = tagNames.filter(n => !existingNames.has(n));

          if (missingNames.length > 0) {
            await prisma.tag.createMany({
              data: missingNames.map((normalizedName, index) => ({
                userId: job.data.userId,
                name: result.tagging.tags.find(t => t.toLowerCase().trim() === normalizedName) || normalizedName,
                normalizedName,
              })),
              skipDuplicates: true,
            });
            ...
            const newTags = await prisma.tag.findMany({
              where: {
                userId: job.data.userId,
                normalizedName: { in: missingNames }
              }
            });
            existingTags.push(...newTags);
          }

          // 3. Batch link all tags to bookmark (instead of one upsert per tag)
          ...
          console.log(`[Worker] ✓ Tags saved successfully for bookmark ${job.data.bookmarkId} (batched: ${existingTags.length} tags)`);
        }
      ...
      console.warn(`[Worker] ⚠️  No bookmarkId/userId in job data, skipping database save`);
    }
