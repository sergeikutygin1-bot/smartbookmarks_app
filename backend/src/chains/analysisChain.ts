...
/**
 * Analysis Chain - Generates summary and key points from extracted content
 *
...
/**
 * Analyzes extracted content to generate summary and key points
 *
 * @param content - The extracted content to analyze
 * @param config - Optional chain configuration
 * @returns Analysis result with summary and key points
 */
export async function analyzeContent(
  ...
  try {
    ...
    return {
      summary: `Failed to analyze content from ${content.domain}. The content appears to be about: ${content.title}`,
      keyPoints: [
 ...
 *
 * @param contents - Array of extracted content
 * @param config - Optional chain configuration
...
export async function analyzeContentBatch(
  ...
  for (const content of contents) {
    const result = await analyzeContent(content, config);
    results.push(result);
  }
